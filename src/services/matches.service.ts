...
export const fetchMatches = async (params?: { year?: number; take?: number; skip?: number }): Promise<Match[]> => {
  const res = await api.get('/matches', { params });
  return res.data;
};

export const fetchMatch = async (id: string): Promise<Match & { events: MatchEvent[] }> => {
...
export const updateMatch = async (id: string, payload: Partial<Omit<Match, 'id'>>) => {
...
export const addMatchEvent = async (matchId: string, payload: Partial<MatchEvent> & { type: MatchEvent['type'] }) => {
...
export const setLineup = async (
  matchId: string,
  entries: { playerId: string; role: 'starter' | 'sub'; inMinute?: number; outMinute?: number }[],
) => {
...
export const setAttendance = async (
  matchId: string,
  attendances: { playerId: string; status: 'present' | 'absent'; note?: string }[],
) => {
