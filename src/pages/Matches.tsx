...
import { addMatchEvent, setLineup, updateMatch } from '../services/matches.service';
import { useNavigate } from 'react-router-dom';
import { useAuthStore } from '../store/auth.store';
import { formatMatchDate } from '../utils/date';

export default function Matches() {
  const { data, isLoading, error, fetchNextPage, hasNextPage, isFetchingNextPage } = useInfiniteMatches(undefined, 10);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setForm({ opponentName: '', date: '', location: '', competitionId: '', type: 'friendly' });
  };

  const saveScore = useMutation({
    mutationFn: (payload?: any) => updateMatch(finalizingId as string, { scoreFor, scoreAgainst, ...(payload || {}) }),
    onSuccess: () => {
  ...
  const handleFinalize = async () => {
    const goals = localEvents.filter((ev: any) => ev.type === 'goal' || ev.type === 'assist').length;
    if (scoreFor === undefined || scoreAgainst === undefined) {
      setModalError('Informe o placar antes de finalizar.');
      return;
    }
    if (goals !== scoreFor) {
      setModalError('Total de gols nos eventos deve bater com o placar do time.');
      return;
    }
    ...
    const updatedMatch = await saveScore.mutateAsync({ status: 'finalized', scoreFor, scoreAgainst } as any);
    ...
    lists.forEach(([key, data]) => {
      if (data?.pages) {
        const newPages = data.pages.map((page: any[]) =>
          page.map((m) => (m.id === finalizingId ? { ...m, ...updatedMatch, scoreFor, scoreAgainst, status: 'finalized' } : m)),
        );
        qc.setQueryData(key, { ...data, pages: newPages });
      } else if (Array.isArray(data)) {
        qc.setQueryData(
          key,
          data.map((m) => (m.id === finalizingId ? { ...m, ...updatedMatch, scoreFor, scoreAgainst, status: 'finalized' } : m)),
        );
      }
    });
    ...
    <div className="space-y-6">
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-slate-900 via-slate-800 to-indigo-900 p-8 shadow-xl">
        ...
        <div className="relative flex flex-col gap-2 text-white">
          ...
          <p className="text-sm text-slate-200">Planeje, finalize e revise seus confrontos.</p>
          <div className="mt-4 rounded-2xl bg-white/10 p-4 backdrop-blur">
              ...
              <Select
                ...
                onChange={(e) => {
                  ...
                  setForm({
                    ...
                    type: compId ? selected?.type ?? 'friendly' : 'friendly',
                  });
                }}
                className="bg-white/90 text-slate-900"
              >
                <option value=""></option>
                <option value="create-competition" className="text-slate-500">Cadastre uma competição</option>
                {competitions?.map((c) => (
            ...
            {!competitions?.length && (
              <div className="mt-2 text-xs text-slate-200/90">
                Nenhuma competição cadastrada. Vá em "COMPETIÇÕES" para criar e depois selecione aqui.
              </div>
            )}
            {isLoading && <div className="mt-3 text-white/80">Carregando...</div>}
            {error && <div className="mt-3 text-sm text-red-200">Erro ao carregar jogos</div>}
          </div>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-6xl grid gap-4 md:grid-cols-2">
            ...
            >
              ...
              >
                <div className="space-y-1">
                  <div className="text-lg font-bold text-slate-900">{teamName.toUpperCase()} x {m.opponentName.toUpperCase()}</div>
                  <div className="text-xs text-slate-600">{formatMatchDate(m.date)}</div>
        ...
        <div ref={loadMoreRef} />
        {isFetchingNextPage && <div className="text-center text-sm text-slate-500">Carregando...</div>}
      </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-2 text-sm">
              ...
              <input
                ...
                value={scoreFor ?? ''}
                onChange={(e) => setScoreFor(e.target.value === '' ? undefined : Number(e.target.value))}
                placeholder="Nós"
              />
              <span>x</span>
              <input
                ...
                className="w-16 rounded border border-slate-300 px-2 py-1"
                value={scoreAgainst ?? ''}
                onChange={(e) => setScoreAgainst(e.target.value === '' ? undefined : Number(e.target.value))}
                placeholder="Eles"
              />
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                ...
                <div className="space-y-2">
                      ...
                      <div className="flex gap-1">
                        <button
                          ...
                          onClick={() => {
                            ...
                            setSelectedSubs(new Set([...selectedSubs].filter((id) => id !== p.id)));
                          }}
                        >
                          Titular
                        </button>
                        <button
                          ...
                          onClick={() => {
                            ...
                            setSelectedStarters(new Set([...selectedStarters].filter((id) => id !== p.id)));
                          }}
                        >
                          Reserva
                        </button>
                  ...
                  {availablePlayers.length === 0 && <div className="text-xs text-slate-500">Nenhum disponível.</div>}
                </div>
              </div>
              ...
              >
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {[...selectedStarters].length === 0 && (
                    <span className="text-xs text-emerald-700">Adicione titulares acima.</span>
                  )}
                </div>
              </div>
              ...
              <div className="rounded-xl border border-blue-200 bg-blue-50 p-3">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {[...selectedSubs].length === 0 && (
                    <div className="text-xs text-blue-700">Selecione reservas acima.</div>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap items-center gap-2 pb-2">
                ...
                {eventType === 'assist' && (
                  <select className="rounded border border-slate-300 px-2 py-1 text-sm" value={assistId ?? ''} onChange={(e) => setAssistId(e.target.value || undefined)}>
                ...
                {(eventType === 'yellow' || eventType === 'red') && (
                  <select className="rounded border border-slate-300 px-2 py-1 text-sm" value={cardPlayerId ?? ''} onChange={(e) => setCardPlayerId(e.target.value || undefined)}>
                ...
                <input
                  ...
                  placeholder="Min"
                  value={minute ?? ''}
                  onChange={(e) => setMinute(e.target.value ? Number(e.target.value) : undefined)}
                />
                <button
                  onClick={() => {
                    ...
                    if (eventType === 'assist' && !scorerId) {
                      setModalError('Selecione o autor do gol para registrar assistência.');
                      return;
                    }
                    if ((eventType === 'yellow' || eventType === 'red') && !cardPlayerId) {
                    ...
                    const newEv = {
                      ...
                      scorer: selectedPlayers.find((p: any) => p.id === scorerId),
                      assist: selectedPlayers.find((p: any) => p.id === assistId),
                      player: selectedPlayers.find((p: any) => p.id === cardPlayerId),
                      isLocal: true,
                    };
