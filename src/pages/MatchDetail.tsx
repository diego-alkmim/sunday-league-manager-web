...
export default function MatchDetail() {
  ...
  useEffect(() => {
    if (data?.attendance) {
      const presentIds = data.attendance.filter((a: any) => a.status === 'present').map((a: any) => a.playerId);
      setPresent(new Set(presentIds));
    }
    if (data?.lineups) {
      const starterIds = data.lineups.filter((l: any) => l.role === 'starter').map((l: any) => l.playerId);
  ...
  const attendanceMutation = useMutation({
    mutationFn: (attendances: any[]) => setAttendance(id as string, attendances),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['matches', id] }),
  });
  ...
  return (
    <div className="space-y-3">
      ...
      {error && <div className="text-sm text-red-600">Erro ao carregar jogo</div>}
      {data && (
        <>
          <Card title="Resumo">
          ...
          <Card title="Eventos">
            ...
            {data.events?.length === 0 && <div className="text-sm text-slate-500">Sem eventos.</div>}
          ...
          <Card title="Escalação">
            <div className="text-sm text-slate-600 mb-2">Marque titulares; demais viram reservas.</div>
