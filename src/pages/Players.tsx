...
export default function Players() {
  ...
  const [form, setForm] = useState({ name: '', nickname: '', position: '', foot: '', number: '' });
  const [editing, setEditing] = useState<string | null>(null);
  const [editForm, setEditForm] = useState({ name: '', nickname: '', position: '', foot: '', number: '' });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setForm({ name: '', nickname: '', position: '', foot: '', number: '' });
  };
    ...
    <div className="space-y-6">
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-slate-900 via-slate-800 to-indigo-900 p-8 shadow-xl">
        ...
        <div className="relative flex flex-col gap-2 text-white">
          ...
          <p className="text-sm text-slate-200">Cadastre, edite e gerencie seu elenco com uma pegada futurista.</p>
          ...
          editing === p.id ? (
            <Card key={p.id} className="shadow-lg border border-slate-200">
              ...
              >
                ...
                <Input value={editForm.position} onChange={(e) => setEditForm({ ...editForm, position: e.target.value })} label="Posicao" />
                <Input value={editForm.foot} onChange={(e) => setEditForm({ ...editForm, foot: e.target.value })} label="Perna" />
                <Input value={editForm.number} onChange={(e) => setEditForm({ ...editForm, number: e.target.value })} label="Numero" type="number" />
            ...
            <div
              key={p.id}
              className="flex items-center justify-between rounded-xl border border-slate-200 bg-white/90 px-5 py-4 shadow-md hover:border-indigo-200 hover:shadow-lg transition"
            >
