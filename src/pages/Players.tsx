...
export default function Players() {
  ...
  const [form, setForm] = useState({ name: '', nickname: '', position: '', foot: '', number: '', photoUrl: '' });
  const [editing, setEditing] = useState<string | null>(null);
  const [editForm, setEditForm] = useState({ name: '', nickname: '', position: '', foot: '', number: '', photoUrl: '' });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setForm({ name: '', nickname: '', position: '', foot: '', number: '', photoUrl: '' });
  };

  const renderAvatar = (p: any) => {
    if (p.photoUrl) {
      return <img src={p.photoUrl} alt={p.name} className="h-14 w-14 rounded-full object-cover ring-2 ring-slate-200" />;
    }
    ...
    <div className="space-y-6">
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-slate-900 via-slate-800 to-indigo-900 p-8 shadow-xl">
        ...
        <div className="relative flex flex-col gap-2 text-white">
          ...
          <p className="text-sm text-slate-200">Cadastre, edite e gerencie seu elenco.</p>
          <div className="mt-4 rounded-2xl bg-white/10 p-4 backdrop-blur">
            <form className="grid grid-cols-1 items-end gap-3 md:grid-cols-12" onSubmit={handleSubmit}>
              <Input
                label="Nome"
            ...
            {error && <div className="mt-3 text-sm text-red-200">Erro ao carregar jogadores</div>}
          </div>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-5xl grid gap-3 md:grid-cols-2">
              ...
              >
                <Input value={editForm.name} onChange={(e) => setEditForm({ ...editForm, name: e.target.value })} label="Nome" />
                <Input value={editForm.nickname} onChange={(e) => setEditForm({ ...editForm, nickname: e.target.value })} label="Apelido" />
                <Select value={editForm.position} onChange={(e) => setEditForm({ ...editForm, position: e.target.value })} label="Posicao">
                  <option value=""></option>
                  {positions.map((pos) => (
                    <option key={pos} value={pos}>
                      {pos}
                ...
                <Select value={editForm.foot} onChange={(e) => setEditForm({ ...editForm, foot: e.target.value })} label="Perna">
                  <option value=""></option>
                  {feet.map((f) => (
                    <option key={f} value={f}>
                      {f}
                ...
                <Select value={editForm.number} onChange={(e) => setEditForm({ ...editForm, number: e.target.value })} label="Numero">
                  <option value=""></option>
                  {numbers.map((n) => (
                    <option key={n} value={n}>
                      {n}
                ...
                <div
                  ...
                  className={`rounded-xl border border-slate-200 bg-white/90 px-5 py-4 shadow-md hover:border-indigo-200 hover:shadow-lg transition ${
                    canManage ? 'flex items-center justify-between' : 'flex flex-col items-center text-center gap-3'
                  }`}
                >
                  <div className={`flex items-center gap-3 w-full ${canManage ? '' : 'justify-center'}`}>
                    {renderAvatar(p)}
                    <div className={canManage ? 'space-y-1' : 'space-y-1 text-center'}>
                      <div className="text-lg font-bold text-slate-900">{p.name}</div>
                      <div className="text-xs text-slate-600">
                        {p.position} {p.number ? `#${p.number}` : ''}
                      </div>
                      {p.nickname && <div className="text-xs text-indigo-700">{p.nickname}</div>}
                    </div>
                  </div>
                  {canManage && (
